...
//Authors: Haydn Thurman and Grange Simpson
  ...
  public static class Networking
  {
    ...
    private static void AcceptNewClient(IAsyncResult ar)
    {
        ...
        Tuple<Action<SocketState>, TcpListener> tuple = (Tuple<Action<SocketState>, TcpListener>) ar.AsyncState;
            ...
            try
            {
                //retrieving the listener from the tuple
                TcpListener listener = tuple.Item2;

                //creating a socket through the listener
                Socket newSocket = listener.EndAcceptSocket(ar);

                //creating a socket state and invoking OnNetworkAction delegate
                SocketState newClient = new SocketState(tuple.Item1, newSocket);
            ...
            {
                SocketState state = new SocketState(tuple.Item1, null);
                state.ErrorOccurred = true;
                state.ErrorMessage = "There was an error connecting: " + e.Message;
                tuple.Item1(state);
            }
    ...
    public static void ConnectToServer(Action<SocketState> toCall, string hostName, int port)
    {
      ...
      try
      {
        ...
        if (!foundIPV4)
        {
                    ...
                    errorState.ErrorMessage = "There was an error connecting to the IPV4 Address.";
                    toCall(errorState);
                }
      }
      catch (Exception)
      {
        ...
        catch (Exception)
        {
                    ...
                    SocketState errorState = new SocketState(toCall, null);
                    errorState.ErrorOccurred = true;
                    errorState.ErrorMessage = "There was an error connecting to the IP Address.";
                    toCall(errorState);
                }
      }
      ...
      socket.NoDelay = true;
            ...
            try
            {
                ...
                if (!socket.Connected)
                {
                    SocketState errorState = new SocketState(toCall, null);
                    errorState.ErrorOccurred = true;
                    errorState.ErrorMessage = "There has been a timeout error.";
                    toCall(errorState);
                }
            }
            //If anything goes wrong when trying to connect
            catch (Exception e)
            {
                SocketState errorState = new SocketState(toCall, null);
                errorState.ErrorOccurred = true;
                errorState.ErrorMessage = "There was an error while trying to connect the client to the server: " + e.Message;
                toCall(errorState);
            }
        }
    ...
    private static void ReceiveCallback(IAsyncResult ar)
    {
            ...
            //If an error occurs during the receive process
            catch
            {
                state.ErrorOccurred = true;
                state.ErrorMessage = "There was an error processing data.";
                state.OnNetworkAction(state);
            }
    }
    ...
    {
            bool sent = false;
            //Making sure the socket is connected
            if (socket.Connected)
            {
                try
                {
    ...
    {
            bool sent = false;
            //Making sure socket is connected
